...
const Main = (props) =>{
  ...
  const callAPI = async (e,text,) =>{
    ...
    //  console.log('api get called')
      if(text){
        ...
        // setWrongWords(highlightWords)
        highlightWordsHandler(highlightWords , arr )
        // contentEditable.current.innerHTML="<div>need to handle place holder</div>"
      }
    }
  }
  const highlightWordsHandler = (highlightWords , arr) =>{
      ...
      // console.log('enter')
       if (((highlightWords.hasOwnProperty(key)) && highlightWords[key]['highlighted'] === false) || (highlightWords[key]['count'] < wrongWords[key]['count'])) {
        ...
        if((childrenDiv.match(key)) ){
            ...
            let rplc = `<span class='error' id=${aIndex} ><label>${key}</label></span>`
            let pattren = `<label>(\s?\w+<)` ;
            let myregex = new RegExp(pattren);
            // console.log(childrenDiv  , "mytest" , key );
            let rex   =  new RegExp(key , 'g') ;
            contentEditable.current.children[0].innerHTML = (contentEditable.current.children[0].innerHTML).replace( rex   , rplc);
  ...
  const  setCursorAfterElement =(ele) =>{
    ...
    if (ele) {
      ...
      let dummyText = dummyElement.innerText ;
      ele.parentNode.appendChild(dummyElement)
      dummyElement.replaceWith(dummyText);
     console.log(ele.parentNode,'parent node');
     let position = {start:caretPosition.start+2,end:caretPosition.end+2}
