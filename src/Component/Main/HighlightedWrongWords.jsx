...
class HighlightedWrongWords extends Component {
    ...
    getSuggestedWords = (obj, list, decoratedWord) => {
        obj = this.props.correctionsList ? this.props.correctionsList : {};
    ...
    render() {
	  ...
	  var threeSuggesteddLists = this.getSuggestedWords().map(e => <ul>{e.slice(0, 3).map(word => <a onClick={()=>this.test(word)} ref='dummy'> {word} </a>)}</ul>);

		var allSuggesteddLists = this.getSuggestedWords().map(e => 
		    <nav className="acc_sugg_words"> {e.map(word => <a> {word} </a>)}</nav> ); 
		console.log('all ' , allSuggesteddLists);
